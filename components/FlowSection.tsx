  ...
  const steps = [
    {
      number: "01",
      title: "資料請求・お問い合わせ",
      description: "まずは、LLM 可視化の概要や成功事例、SaaS とコンサルティングの違いが分かる資料をダウンロード。自社のマーケティング・営業戦略にどのように活用できるのかを具体的にイメージできます。",
      icon: "mail"
    },
    {
      number: "02",
      title: "ヒアリング・無料相談",
      description: "現状の集客・問い合わせ状況、強化したい事業やサービス、狙いたいキーワード・ユースケースについて詳しくヒアリング。経験豊富なコンサルタントが、SaaSのみ／コンサル併用 など最適なプランをご提案します。",
      icon: "chat"
    },
    {
      number: "03",
      title: "契約・アカウント発行",
      description: "ご提案内容にご納得いただけましたら、契約手続きを実施。貴社専用のワークスペースとアカウントを発行し、初期設定や基本的な使い方をオンラインでご案内します。",
      icon: "document"
    },
    {
      number: "04",
      title: "サイトクロール・LLM診断開始",
      description: "Firecrawl を用いて自社サイト全体をクロールし、同時に選定したプロンプト群で主要 LLM への自動クエリを開始。「どのプロンプトで自社が言及されているか」「どのコンテンツが LLM に効いているか」を初回レポートとしてご共有します。",
      icon: "search"
    },
    {
      number: "05",
      title: "ダッシュボード共有・改善提案",
      description: "LLM 可視化ダッシュボードへのログイン情報を共有し、画面を見ながら現状の課題と改善の方向性を解説。SaaS利用のみの場合は社内で回せる改善アクションを、コンサル併用の場合は新規ページ案・リライト方針・優先順位を一緒に設計していきます。",
      icon: "people"
    },
    {
      number: "06",
      title: "定期モニタリング・レポーティング",
      description: "導入後は、日次の自動計測と月次のサマリーレポートで継続的にフォロー。「新しく公開したコンテンツが LLM にどう反映されたか」「競合の露出変化に対してどう手を打つべきか」を定期的に確認しながら、AI検索時代における「選ばれ続けるブランド」づくりを支援します。",
      icon: "check"
    }
  ];
          ...
          <div className="relative inline-block">
            <div className="bg-[#F4F563] px-4 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl shadow-md">
              <p className="text-gray-800 text-sm md:text-lg font-bold text-center leading-relaxed">
                簡単6ステップで本格導入が可能！<br className="md:hidden" />最短2ヶ月で自社専用の LLM 可視化ダッシュボードを構築
              </p>
            </div>
            {/* 吹き出しの尾 */}
