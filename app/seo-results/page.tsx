...
export default function SEOResultsPage() {
  ...
  useEffect(() => {
    ...
    const pollStatus = async () => {
      try {
        const response = await fetch(`/api/seo-check/status?jobId=${encodeURIComponent(jobId)}`);
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-lg shadow-md p-8">
            <div className="text-center">
              ...
              <h1 className="text-2xl font-bold text-gray-900 mb-4">ウェブサイトを解析中...</h1>
                ...
                <div className="mt-6">
                  <div className="w-full bg-gray-200 rounded-full h-2.5 mb-2">
                    <div 
                      ...
                      style={{ width: `${crawlStatus.total > 0 ? (crawlStatus.completed / crawlStatus.total * 100) : 0}%` }}
                    ></div>
                  </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-lg shadow-md p-8">
            <div className="text-center">
              ...
              <h1 className="text-2xl font-bold text-gray-900 mb-4">クロール処理に失敗しました</h1>
  ...
  const pagesWithTitle = validPages.filter(p => 
    p.title || p.metadata?.title
  ).length;
  const pagesWithDescription = validPages.filter(p => 
    p.metadata?.description
  ).length;
  const pagesWithContent = validPages.filter(p => 
    (p.content || p.markdown || '')?.length > 100
  ).length;

  // Calculate score (0-100)
  const titleScore = totalPages > 0 ? (pagesWithTitle / totalPages) * 35 : 0;
  const descriptionScore = totalPages > 0 ? (pagesWithDescription / totalPages) * 35 : 0;
  const contentScore = totalPages > 0 ? (pagesWithContent / totalPages) * 30 : 0;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-[#3B82F6] text-white py-8 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-2">AI SEO分析結果</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-8 mb-8">
          <div className="text-center mb-8">
            <h2 className="text-xl font-bold text-gray-900 mb-4">総合SEOスコア</h2>
            <div className={`text-7xl font-bold ${getScoreColor(totalScore)} mb-2`}>
              {totalScore}
              <span className="text-3xl">/100</span>
            </div>
            <p className="text-gray-600">
              {totalScore >= 80 && '優秀なSEO対策がされています！'}
              {totalScore >= 60 && totalScore < 80 && '良好ですが、改善の余地があります'}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-3xl font-bold text-[#3B82F6] mb-2">{totalPages}</div>
              <div className="text-sm text-gray-600">ページ数</div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-3xl font-bold text-[#3B82F6] mb-2">
                {totalPages > 0 ? Math.round((pagesWithTitle / totalPages) * 100) : 0}%
              </div>
              <div className="text-sm text-gray-600">タイトルあり</div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-3xl font-bold text-[#3B82F6] mb-2">
                {totalPages > 0 ? Math.round((pagesWithDescription / totalPages) * 100) : 0}%
              </div>
        ...
        {/* Page List */}
        <div className="bg-white rounded-lg shadow-md p-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">検出されたページ</h2>
                ...
                <div key={index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex gap-2 ml-4">
                      {title !== 'タイトルなし' && (
                        <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">✓ Title</span>
                      )}
                      {description && (
                        <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">✓ Desc</span>
                      )}
                      {contentLength > 100 && (
                        <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">✓ Content</span>
                      )}
                    </div>
                  </div>
                  <p className="text-sm text-[#3B82F6] mb-2 break-all">{page.url}</p>
                  {description && (
        ...
        <div className="bg-[#3B82F6] text-white rounded-lg shadow-md p-8 mt-8 text-center">
          <h2 className="text-2xl font-bold mb-4">LLM可視化でさらに詳しく分析</h2>
          <p className="mb-6">
